...
@Component({
  ...
  templateUrl: './table-dinamico-modal.component.html',
  styleUrl: './table-dinamico-modal.component.css'
})
export class TableDinamicoModalComponent implements OnInit{
  ...
  @Output() _datoEvento = new EventEmitter<{_id : number }>();
  @Output() _datoEliminar = new EventEmitter<{_id : number }>();
  @Output() _datoConsulta = new EventEmitter<{_id : number }>();
  ...
  */
  ...
  ObtenerIdEvento(_id: string) {
    this._datoEvento.emit({ _id: parseInt(_id) });
  }

  EliminarIdEvento(_id: string) {
    this._datoEliminar.emit({ _id: parseInt(_id) });
  }

  ConsultaId(_id: string) {
    this._datoConsulta.emit({ _id: parseInt(_id) });
  }
